---
layout: archive
title: "Talks"
permalink: /presentations/
author_profile: true
redirect_from:
  - /resume
---

{% include base_path %}
<div style="display: flex; align-items: flex-start; margin-bottom: 20px;">
    <img src="https://keli97.github.io/files/conference/mobicom_2024.png" alt="MobiCom 2024" style="width: 80px; height: auto; margin-right: 15px; margin-top: 50px;">
    <div>
        <h2>MobiCom 2024</h2>
        <p><strong>November 2024, Washington, D.C.</strong></p>
        <p>Presentation title: <i>GazeTrak: Exploring Acoustic-based Eye Tracking on a Glass Frame</i></p>
    </div>
</div>

<figure>
    <center><img src="https://keli97.github.io/files/mobicom24.PNG" alt="Trulli" style="width:80%" class="center"></center>
</figure>

<div style="display: flex; align-items: flex-start; margin-bottom: 20px;">
    <img src="https://keli97.github.io/files/university/cornell.png" alt="Cornell" style="width: 80px; height: auto; margin-right: 15px; margin-top: 40px;">
    <div>
        <h2>Admission to Candidacy (A Exam) Talk</h2>
        <p><strong>Sept. 2024, Ithaca, NY</strong></p>
        <p>Presentation title: <i>AI-Powered Wearables for Face and Eye Tracking: Towards Real-World Health Applications</i></p>
    </div>
</div>

<figure>
    <center><img src="https://keli97.github.io/files/aexam.JPG" alt="Trulli" style="width:80%" class="center"></center>
</figure>

<div style="display: flex; align-items: flex-start; margin-bottom: 20px;">
    <img src="https://keli97.github.io/files/university/uoftokyo.png" alt="The University of Tokyo" style="width: 80px; height: auto; margin-right: 15px; margin-top: 40px;">
    <div>
        <h2>Presentation at Kawahara Laboratory, the University of Tokyo</h2>
        <p><strong>May 2024, Tokyo, Japan</strong></p>
        <p>Research talk on <a href="https://www.akg.t.u-tokyo.ac.jp/archives/5076">acoustic sensing around the body</a></p>
    </div>
</div>

<div style="display: flex; align-items: flex-start; margin-bottom: 20px;">
    <img src="https://keli97.github.io/files/conference/chi_2024.png" alt="CHI 2024" style="width: 80px; height: auto; margin-right: 15px; margin-top: 40px;">
    <div>
        <h2>CHI 2024</h2>
        <p><strong>May 2024, Honolulu, HI</strong></p>
        <p>Presentation title: <i>EyeEcho: Continuous and Low-power Facial Expression Tracking on Glasses</i></p>
    </div>
</div>

<figure>
    <center><img src="https://keli97.github.io/files/chi24.jpg" alt="Trulli" style="width:80%" class="center"></center>
</figure>

<div style="display: flex; align-items: flex-start; margin-bottom: 20px;">
    <img src="https://keli97.github.io/files/university/cornell.png" alt="Cornell" style="width: 80px; height: auto; margin-right: 15px; margin-top: 40px;">
    <div>
        <h2>Presentation at the Cornell Computer System Lab (CSL)</h2>
        <p><strong>Mar. 2023, Ithaca, NY</strong></p>
        <p>Presentation on <i>EarIO</i> as part of the research introduction of SciFi Lab</p>
    </div>
</div>

<figure>
    <center><img src="https://keli97.github.io/files/csl.JPG" alt="Trulli" style="width:80%" class="center"></center>
</figure>

<div style="display: flex; align-items: flex-start; margin-bottom: 20px;">
    <img src="https://keli97.github.io/files/conference/ubicomp_2022.png" alt="UbiComp 2022" style="width: 80px; height: auto; margin-right: 15px; margin-top: 50px;">
    <div>
        <h2>UbiComp/ISWC 2022</h2>
        <p><strong>Sept. 2022, Atlanta, GA</strong></p>
        <p>Presentation title: <i>EarIO: A Low-power Acoustic Sensing Earable for Continuously Tracking Detailed Facial Movements</i></p>
    </div>
</div>

<figure>
    <center><img src="https://keli97.github.io/files/ubicomp22.JPG" alt="Trulli" style="width:80%" class="center"></center>
</figure>
